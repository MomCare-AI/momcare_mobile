...
void main() {
  group('VitalsRepository.fetchReadings', () {
    ...
    test(
      'returns readings newest-first, matching the real endpoint\'s ordering',
      () async {
        final repository = VitalsRepository();
        final farPast = DateTime.now().subtract(const Duration(days: 3650));
        ...
        for (var i = 0; i < readings.length - 1; i++) {
          expect(
            readings[i].recordedAt.isAfter(readings[i + 1].recordedAt) ||
                readings[i].recordedAt.isAtSameMomentAs(
                  readings[i + 1].recordedAt,
                ),
            isTrue,
            reason:
                'Reading at index $i is not newer than or equal to the next one',
          );
        }
      },
    );
